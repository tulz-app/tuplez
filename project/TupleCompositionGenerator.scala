...
class TupleCompositionGenerator(sourceManaged: File)
  ...
  def doGenerate(): Unit = {
    ...
    leave("}")
    println()
    enter("""trait Composition_PriLowest {""")
    ...
    enter("""trait Composition_PriMed extends Composition_PriLow {""")
    println("""implicit def _toA[A] = Composition[Unit, A, A]((_, a) => a)""")
    println("""implicit def Ato_[A] = Composition[A, Unit, A]((a, _) => a)""")
    leave("""}""")
    println()
    enter("""object Composition extends Composition_PriMed {""")
  ...
  def generatePriLow(): Unit = {
    ...
    println()
    println("""implicit def `T1+scalar`[L, R: NonTuple]: Composition[Tuple1[L], R] = Composition((l, r) => (l._1, r))""")
    println()
    println("""implicit def `scalar+T1`[L: NonTuple, R]: Composition[L, Tuple1[R]] = Composition((l, r) => (l, r._1))""")
    println()

    def forSizeAndScalar(size: Int): Unit = {
      val left = tupleType(size - 1)
      enter(s"""implicit def `T${size - 1}+scalar`[${left}, R: NonTuple]: Composition[(${left}), R] =""")
      enter(
        s"""Composition((l, r) =>"""
      )
    ...
    def forScalarAndSize(size: Int): Unit = {
      ...
      enter(s"""implicit def `scalar+T${size - 1}`[L: NonTuple, ${right}]: Composition[L, (${right})] =""")
      enter(
        s"""Composition((l, r) =>"""
      )
    ...
    println("""implicit def `T1+T1`[L, R]: Composition[Tuple1[L], Tuple1[R]] = Composition((l, r) => (l._1, r._1))""")

    def forSizeAnd1(size: Int): Unit = {
      val left = tupleType(size)
      enter(s"""implicit def `T${size}+T1`[${left}, R]: Composition[(${left}), Tuple1[R]] =""")
      enter(
        s"""Composition((l, r) =>"""
      )
    ...
    def for1AndSize(size: Int): Unit = {
      ...
      enter(s"""implicit def `T1+T${size}`[L, ${right}]: Composition[Tuple1[L], (${right})] =""")
      enter(
        s"""Composition((l, r) =>"""
      )
    ...
    def forSizes(size1: Int, size2: Int): Unit = {
      ...
      enter(s"""implicit def `T${size1}+T${size2}`[${left}, ${right}]: Composition[(${left}), (${right})] =""")
      enter(
        s"""Composition((l, r) =>"""
      )
